...
using Emgu.CV;
using Emgu.CV.Structure; //Bgr, mCvFont
using Emgu.CV.CvEnum; //Font
using Emgu.CV.UI; //ImageViewer
using Emgu.Util; //Using
    ...
    public partial class Form1 : Form
    {
        ...
        private void OpenItem_Click(object sender, EventArgs e)
        {
            ...
            if(ofdOpen.ShowDialog() == DialogResult.OK)
            {
                ...
                imageBox1.Image = bgrImageObj;
                imgeObj = bgrImageObj.Convert<Gray, byte>();
        ...
        */
        ...
        private void Increase1Item_Click(object sender, EventArgs e)
        {
            ...
            imgeObj = imgeObj.Convert<byte>(delegate (byte b)
            {
       ...
       /* void adjustContrastS()
        ...
        private void DeceaseItem_Click(object sender, EventArgs e)
        {
            ...
            imgeObj = imgeObj.Convert<byte>(delegate (byte b)
            {
                return (byte)((255.0 - 2.0 * Alpha) / 255 * b + Alpha);
            });
            imageBox1.Image = imgeObj;
        }
        ...
        private void CapItem_Click(object sender, EventArgs e)
        {
            imgeObj = imgeObj.Convert<byte>(delegate (byte b)
            {
                return (byte)(255.0 - 255.0 *(b/127.0-1.0));
            });
            imageBox1.Image = imgeObj;
            /*int x, y, index;
            int[] LUT = new int[256];
        ...
        */
        ...
        private void XorItem_Click(object sender, EventArgs e)
        {
            //xor => 같으면 0, 다르면 1 이다. -> 같은것 마스크 할때 쓰인다.
            Gray gray = new Gray(106.0);
            //106 = 0110 1010
            //106.0 => 실수로 인식한다.  0110 1010 + 0100 0000(64) => 1010 1010
            //double (8바이트) -> 부호비트 1비트 + 지수비트 11비트 + 소수값 52비트 
            imgeObj = imgeObj.Xor(gray);
            imageBox1.Image = imgeObj;


        }

        private void AndItem_Click(object sender, EventArgs e)
        {
            //and => 둘다 1일때만 1 -> 특정 영역을 0으로 만들때 
            Gray gray = new Gray(106.0);
            imgeObj = imgeObj.And(gray);
            imageBox1.Image = imgeObj;
        }

        private void OrItem_Click(object sender, EventArgs e)
        {
            //or => 하나만 1일때 1 -> 특정 영역을 1으로 만들때 
            Gray gray = new Gray(106.0);
            imgeObj = imgeObj.Or(gray);
            imageBox1.Image = imgeObj;
        }

        private void EqualizationItem_Click_1(object sender, EventArgs e)
        {
            imgeObj._EqualizeHist();
            imageBox1.Image = imgeObj;
        }
    }
}
